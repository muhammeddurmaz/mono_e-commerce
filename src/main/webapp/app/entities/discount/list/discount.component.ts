...
import { EntityArrayResponseType, DiscountService } from '../service/discount.service';
...
export class DiscountComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['title', 'image', 'description'];
  ...
  trackId = (_index: number, item: IDiscount): number => this.discountService.getDiscountIdentifier(item);

  search(query: string): void {
    if (query && DiscountComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (DiscountComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.discountService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
