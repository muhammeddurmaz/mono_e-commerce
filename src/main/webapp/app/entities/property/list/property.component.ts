...
import { EntityArrayResponseType, PropertyService } from '../service/property.service';
...
export class PropertyComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProperty): number => this.propertyService.getPropertyIdentifier(item);

  search(query: string): void {
    if (query && PropertyComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (PropertyComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.properties = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IProperty[] | null): IProperty[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.propertyService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.propertyService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
