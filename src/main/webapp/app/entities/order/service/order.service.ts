...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'placedDate'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      placedDate: order.placedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      placedDate: restOrder.placedDate ? dayjs(restOrder.placedDate) : undefined,
    };
  }
