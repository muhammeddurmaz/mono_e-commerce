...
export class ProductComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['barcode', 'modelCode', 'name', 'description', 'image', 'sizee'];
  ...
  search(query: string): void {
    if (query && ProductComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.productService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
