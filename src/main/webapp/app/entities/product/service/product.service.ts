...
export class ProductService {
  ...
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/_search/products');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<RestProduct[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      addedDate: product.addedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      addedDate: restProduct.addedDate ? dayjs(restProduct.addedDate) : undefined,
    };
  }
