...
import { EntityArrayResponseType, FavoriteService } from '../service/favorite.service';
...
export class FavoriteComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFavorite): number => this.favoriteService.getFavoriteIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.favorites = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFavorite[] | null): IFavorite[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.favoriteService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.favoriteService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
