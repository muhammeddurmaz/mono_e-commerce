...
import { EntityArrayResponseType, ProductTypeService } from '../service/product-type.service';
...
export class ProductTypeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProductType): number => this.productTypeService.getProductTypeIdentifier(item);

  search(query: string): void {
    if (query && ProductTypeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (ProductTypeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.productTypes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IProductType[] | null): IProductType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.productTypeService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.productTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
