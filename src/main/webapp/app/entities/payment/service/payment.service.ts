...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'placedDate'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      placedDate: payment.placedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      placedDate: restPayment.placedDate ? dayjs(restPayment.placedDate) : undefined,
    };
  }
